...
const _C_DANGER := Color(1.0, 0.0, 0.188, 1.0)
const _C_TEXT_ON_BTN := Color.BLACK

# ── Ready ────────────────────────────────────────────────────────────────────
func _ready() -> void:
    _set_dialogue_input_block(false)
...
func _set_dialog_visible(v: bool) -> void:
    ...
    # === КРИТИЧНО: БЛОКИРУЕМ ДИАЛОГ ===
    _dialogue_blocked = v
    _set_dialogue_input_block(v)
    
func _set_dialogue_input_block(blocked: bool) -> void:
    # Можно через SceneTree или через сигнал, но самый надёжный способ:
    var game_root = get_tree().root
    if blocked:
        game_root.set_meta("dialogue_input_blocked", true)
    else:
        game_root.set_meta("dialogue_input_blocked", false)
# ── Обработчики ───────────────────────────────────────────────────────────────
